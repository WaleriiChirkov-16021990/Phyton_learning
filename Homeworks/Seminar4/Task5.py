...
def convert_pol(pol):
    ...
    for i in pol:
        ...
        if len(i) == 1:
            i.append(0)
    pol = [list(int(x) for x in j if x != 'x') for j in pol]
    return pol
...
def get_sum_pol(pol):
    ...
    new_pol =\
        [[str(a), str(b), str(c)] for a, b, c in (zip(coefs, var, degrees))]
    for x in new_pol:
        if x[0] == '0':
            del (x[0])
        if x[-1] == '0':
...
def write_to_file(path, pol):
    with open(path, 'w') as data:
        data.write(pol)
    return print(f'Многочлен успешно записан в файл {path}!')
